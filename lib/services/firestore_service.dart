...
class FirestoreService{
  ...
  Future<DocumentReference> addPost(Post post) async{
    ...
    await _posts.add(post.toJson()).then((postRef) => documentReference = postRef);
    return documentReference;
  }

  Future<DocumentReference> addUserToCollection(UserProfile userProfile) async{
    late DocumentReference documentReference;
    await _users.add({userProfile.toJson()}).then((userRef) => documentReference = userRef);
  ...
  Future<UserProfile> getUserFromCollection(String authID) async{
    QuerySnapshot snapshot = await _users.where('authID', isEqualTo: authID).get();
